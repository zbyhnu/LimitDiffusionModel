...
def get_RDPM_prediction(RDPM_predictor, t_cur, t_next, net, unet_enc_out, use_afs, batch_size):
...
def RDPM_sampler(
    ...
    **kwargs
    ...
    """
    RDPM-Solver (https://arxiv.org/abs/2312.00094).
    ...
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):                # 0, ..., N-1
        ...
        r, scale_dir, scale_time = get_RDPM_prediction(RDPM_predictor, t_cur, t_next, net, unet_enc_out, use_afs, batch_size=latents.shape[0])
...
def ipndm_sampler(
    ...
    **kwargs
    ...
    """
    RDPM-Plugin for improved PNDM sampler.
...
def dpm_pp_sampler(
    ...
    **kwargs
    ...
    """
    RDPM-Plugin for multistep DPM-Solver++. 
    ...
    if RDPM_predictor is not None:
        num_steps = 2 * RDPM_predictor.module.num_steps - 1 if train else 2 * num_steps - 1
    for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):                # 0, ..., N-1
        x_cur = x_next
        if RDPM_predictor is not None:
        ...
        t_next_temp = t_mid if RDPM_predictor is not None else t_next
