...
class LocalLimiterUNetModel(UNetModel):
    def forward(self, x, timesteps=None, context=None, local_Limiter=None, **kwargs):
        hs = []
        with torch.no_grad():
            t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
            emb = self.time_embed(t_emb)
        ...
        for module in self.output_blocks:
            h = torch.cat([h, hs.pop() + local_Limiter.pop()], dim=1)
            h = module(h, emb, context)
